...
`dplyr`패키지를 이용해서 `iris` 품종별로 꽃과 꽃받침의 넓이와 길이의 평균을 비교하는 bar그래프를 (error bar 포함) 그려보겠습니다. 이를 위해서는 먼저 iris 데이터를 품종별로 grouping 할 필요가 있습니다. 
...
<!-- ## remove missing \ average the values of the same genes -->
<!-- idx <- which(is.na(gene_names) | duplicated(gene_names)) -->
<!-- ex_data2 <- data.frame(ex_data[-idx,]) -->
<!-- rownames(ex_data2) <- gene_names[-idx] -->

<!-- head(ex_data2)[,1:5] -->
<!-- head(ph_data)[,1:5] -->

<!-- ``` -->
<!-- * BT 수준별로 각 유전자들의 평균 발현양 구하기 -->

<!-- ```{r, eval=F} -->
<!-- tmp <- pivot_longer(ex_data2, everything()) -->
<!-- tmp$name  -->
<!-- ph_data %>% select(columns = "cod", "BT") -->
<!-- colnames(ex_data2) -->
<!-- tmp <- ph_data %>%  -->
<!--   mutate(xcod=paste("X", cod, sep="")) %>%  -->
<!--   dplyr::select(xcod, BT)  -->
